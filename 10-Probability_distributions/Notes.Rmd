...
```{=latex}
The probability that an event happens between an interval $[a,b]$ can be calculated from the PDF $f(x)$ as an integral
...
```{=latex}
Many phenomena in nature follow the normal distribution. It is defined by expected value (and median and mode due to symmetry) $\mu$ and (standard) deviation $\sigma$.
...
# Plot
par(mfrow = c(1, 2))
plot(normal_seq, normal_pmf, ann = FALSE)
plot(normal_seq, normal_cdf, type = "S", ann = FALSE)
```
